...
# temp change , back to your orignal :
ASSETS_PATH = OUTPUT_PATH / Path(r"C:\YusrGP\YusrProject\assets\frame11")
...
def perform_action(event):
    sound = pygame.mixer.Sound(r"C:\YusrGP\YusrProject\assets\frame11\Letter.mp3")
    sound.play()
canvas.tag_bind(image_3, "<Button-1>", perform_action)


#------------button Recording the letter kaf -------------
...
button_1 = Radiobutton(image=button_image_1,
 ...
 variable=radio_var1,
 value=2,
 command=lambda: record_and_save_audio("recorded_audio5.wav"))
...
def open_KafHarakatPage():
    if radio_var1.get() != 0 or radio_var1.get() != 0:
        score = 0
        if radio_var1.get() == 1:
            score += 2
        elif radio_var1.get() == 2:
            AudioRecord(r"C:\YusrGP\YusrProject\recorded_audio5.wav",id1="kaf",id2="ka")
        addScore3(score)
        file_to_run_path = r"C:\YusrGP\YusrProject\KafHarakatPage.py"
        window.destroy()
        subprocess.run(["python", file_to_run_path], check=True)
       #window.destroy()  # Destroy the current window and open page two
       #import KafHarakatPage.py
    else:
...
borderwidth=0,
highlightthickness=0,
command=open_KafHarakatPage, # Call the function to open page two
relief="flat"
